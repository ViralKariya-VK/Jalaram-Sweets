...
document.addEventListener('DOMContentLoaded', function() {
    ...
    proQty.on('click', '.qtybtn', function () {
        var $button = $(this);
        var input = $button.parent().find('input.quantity-input');
        ...
        if ($button.hasClass('inc')) {
            newVal = oldValue + 1;
        } else if (oldValue > 1) { // Set a minimum quantity of 1
            newVal = oldValue - 1;
        }
        input.val(newVal);

        // Calculate new total price for the product row
        var price = parseFloat(input.data('price'));
        var total = newVal * price;
        $button.closest('tr').find('.product-total').text(total.toFixed(2));
    ...
    // Initial calculation of subtotal on page load
    updateSubtotal();
});
